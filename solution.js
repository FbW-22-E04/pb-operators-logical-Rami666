...
const isDogBetter = true, isCatBetter = false;

// 1 Check the result of:

// 1a

console.log(`Q1a: ${isDogBetter && isCatBetter}`); // For an && statement to be true, both its arguments must be simultaneously true. There is no state where true && false can be simultaneously true. Therefore this statement is false

//1b

console.log(`Q1b: ${isDogBetter || isCatBetter}`); // For an || statement to be true, only one argument needs to be true, i.e. isDogBetter OR isCatBetter. Since true is... true..., this statement can be true.

//NOTE: logically, we do not need to check the second argument for truth as the statement has already met a sufficient condition of truth with a true first argument. If, however, the statement was reversed, i.e. isCatBetter || isDogBetter, upon discovering that isCatBetter is false, we would then have to check whether isDogBetter is true before being able to determine whether the statement is true. Computers would approach these statements in a similar logical process.

// 1c

console.log(`Q1c: ${!(isDogBetter && isCatBetter)}`); // note this is the opposite instance of 1a; since true && false cannot be true, so !(true && false) must be true.
